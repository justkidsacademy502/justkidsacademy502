...
if (!isset($_POST['estudiantes']) || empty($_POST['estudiantes'])) {
    session_start();
    $_SESSION['mensaje'] = "No se recibieron datos de observaciones.";
...
$sentencia = $pdo->prepare('
    INSERT INTO observaciones (grado_id, estudiante_id, nivel, bloque, archivo, fyh_creacion, estado)
    VALUES (:grado_id, :estudiante_id, :nivel, :bloque, :archivo, :fyh_creacion, :estado)
');
...
try {
    foreach ($estudiantes as $index => $id_estudiante) {
        ...
        if (!$sentencia->execute()) {
            throw new Exception("No se registraron todas las observaciones");
        }
    }
    ...
    $pdo->commit();
    session_start();
    $_SESSION['mensaje'] = "Se registraron todas las observaciones";
